...
class KNearestNeighbor(object):
  ...
  def compute_distances_one_loop(self, X):
    ...
    for i in range(num_test):
      ...
      dists[i] = [np.sqrt(np.sum((X[i]-x)**2)) for x in self.X_train]
  ...
  def compute_distances_no_loops(self, X):
    ...
    #dists = [[np.sqrt(np.sum((xtr-x)**2)) for x in self.X_train] for xtr in X]
    return dists
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in range(num_test):
      ...
      closest_y = [self.y_train[n] for n in k_nearest]
      ## Identify unique labels and find label with largest number of occurences
      ## np.unique returns a sorted list and
      ## we are returning smallest label in case of ties
